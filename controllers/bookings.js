...
async function create(req, res, next) {
  ...
  booking.save().then(obj => res.status(200).json({
    msg: "Booking almacenado correctamente.",
    obj: obj
...
function list(req, res, next) {
  ...
  Booking.paginate({},options).then(objs => res.status(200).json({
    ...
    msg: "No se pudo obtener la lista de bookings.",
    obj: ex
  }));
}
...
function index(req, res, next) {
  ...
  Booking.findOne({ _id: id }).then((obj) => res.status(200).json({
        msg: `Booking con el id ${id}`,
        obj: obj
      })).catch((ex) => res.status(500).json({
        msg: "No se pudo consultar el booking.",
        obj: ex
      }));
}
...
function replace(req, res, next) {
  ...
  Booking.findOneAndUpdate({ _id: id }, booking, { new: true }).then((obj) => res.status(200).json({
        msg: "Booking remplazado correctamente.",
        obj: obj
      })).catch((ex) => res.status(500).json({
        msg: "No se pudo rempazar el booking.",
        obj: ex
      }));
}
...
function update(req, res, next) {
  ...
  Booking.findOneAndUpdate({ _id: id }, booking).then((obj) => res.status(200).json({
        msg: "Booking actualizado correctamente.",
        obj: obj
      })).catch((ex) => res.status(500).json({
        msg: "No se pudo actualizar el booking.",
        obj: ex
      }));
}
...
function destroy(req, res, next) {
  ...
  Booking.findByIdAndRemove({ _id: id }).then((obj) => res.status(200).json({
        msg: "Booking eliminado correctamente",
        obj: obj
      })).catch((ex) => res.status(500).json({
        msg: "No se pudo eliminar el booking.",
        obj: ex
      }));
}
